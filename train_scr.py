...
def parse_args():
    ...
    parser.add_argument("--augment", action="store_true", help="Bật chế độ Data Augmentation (Random Resized Crop)")
...
def get_transforms(resolution, augment=False):
    transform_list = []
    ...
    return transform_pipeline, transform_pipeline, transform_pipeline
...
def train():
    ...
    content_tf, style_tf, target_tf = get_transforms(args.resolution, augment=args.augment)
    
    if accelerator.is_main_process:
        print(f"[INFO] Data Augmentation Enabled: {args.augment} (RandomResizedCrop)")
